<template>
  <nav class="relative flex items-center justify-between mb-3">
    ...
    <div
      ...
      class="absolute -top-2 left-0 w-full h-screen sm:hidden bg-white-100 z-50 shadow-md"
    >
      <div class="flex flex-col p-4 space-y-4">
        <!-- Close Button -->
        <button
          @click="toggleMobileMenu"
          class="self-end p-2 rounded-lg hover:bg-gray-200 transition-colors"
        >
          <X class="w-6 h-6" />
        </button>
...
function toggleMobileMenu() {
  isNavListOpen.value = !isNavListOpen.value;

  // Add or remove the no-scroll class on the body
  if (isNavListOpen.value) {
